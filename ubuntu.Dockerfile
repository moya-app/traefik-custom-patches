ARG DOCKER_HUB_URL
# WEBUI
FROM ${DOCKER_HUB_URL}library/node:22.9.0-bookworm AS webui
...
# BUILD
FROM --platform=$BUILDPLATFORM ${DOCKER_HUB_URL}library/golang:1.23-bookworm AS gobuild

# See https://docs.docker.com/build/building/multi-platform/#cross-compiling-a-go-application
ARG TARGETOS
ARG TARGETARCH

RUN apt-get update && apt-get install -y --no-install-recommends git mercurial bash gcc musl-dev curl tar ca-certificates tzdata \
...
## IMAGE
FROM ${DOCKER_HUB_URL}library/debian:bookworm-slim

RUN apt-get update && apt-get install -y --no-install-recommends bash curl ca-certificates tzdata \
#    && update-ca-certificates \
    && apt-get clean \
    && rm -rf /var/lib/apt/lists/*
